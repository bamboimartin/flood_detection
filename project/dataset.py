...
def save_csv(dictionary, config, name):
    ...
    df.to_csv(("/home/projects/martin_wd/data"+"/"+name), index=False, header=True)   


def data_path_split(config):
    
    
    paths = pd.read_csv("/home/projects/martin_wd/data/flood-training-metadata.csv")
...
def test_data_path_split(config):
